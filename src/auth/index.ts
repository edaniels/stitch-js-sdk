...
export class Auth {
  ...
  constructor(
    private readonly client: RealmAdminClient,
  ...
  public refreshToken() {
    return this.client
      .doSessionPost()
      .then((json: Record<string, any>) => this.set(json));
  }
  ...
  public handleCookie() {
    ...
    const uaCookie = this.getCookie(authCommon.USER_AUTH_COOKIE_NAME);
    if (!uaCookie) {
      return;
    }
    ...
    this.set(userAuth);
    window.history.replaceState(undefined, "", this.pageRootUrl());
  }
  ...
  public isAccessTokenExpired(
    withinSeconds = authCommon.DEFAULT_ACCESS_TOKEN_EXPIRE_WITHIN_SECS
  ) {
    ...
    return (
      decodedToken.exp &&
      Math.floor(Date.now() / 1000) >= decodedToken.exp - withinSeconds
    );
  }
