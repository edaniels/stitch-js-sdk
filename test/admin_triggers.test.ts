...
import {
  EventSubscription,
  ResourceType
} from "../src/api/v3/EventSubscriptions";
...
function buildEventSubscriptionData(
  name,
  disabled,
  functionId,
  mongodbServiceId
...
function comparePrivateAdminTriggersList(
  actualAdminTriggers,
  expectedAdminTriggers
) {
...
describe("privateAdminTriggers", () => {
  ...
  it("should return all triggers belonging to the app when list is called", async () => {
    ...
    await eventSubscriptions.create(
      buildEventSubscriptionData(
        eventSubscriptionName1,
        false,
        fn.id,
        mongodbService.id
      )
    ...
    comparePrivateAdminTriggersList(
      privateAdminTriggersList,
      expectedPrivateAdminTriggers
    );
  });
  ...
  it("should return a specific trigger when get is called", async () => {
    ...
    const privateAdminTriggersGet = await privateAdminTriggers.get(
  ...
  it("should throw an error when get is called and the trigger does not exist", async () => {
    await expect(privateAdminTriggers.get("1234")).rejects.toEqual(
      new RealmError(
        "must provide valid trigger id (1234): invalid ObjectId",
        ErrUnauthorized
      )
    );
  });
