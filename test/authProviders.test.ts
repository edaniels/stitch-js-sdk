...
describe("Auth Providers", () => {
  ...
  it("invalid create requests should fail", async () => {
    await expect(
      authProviders.create(
        new AuthProviderConfig({ type: AuthProviderType.Userpass })
      )
    ).rejects.toBeDefined();
  });
  ...
  it("enabling auth provider should work", async () => {
    ...
    expect(newAuthProvider.type).toEqual("local-userpass");
    const fetchedProvider = await authProviders
      .authProvider(newAuthProvider.id)
      .get();
  ...
  it("deleting an auth provider should fail if the auth provider is still enabled", async () => {
    ...
    expect(providers).toHaveLength(2);
    await expect(
      authProviders.authProvider(newAuthProvider.id).remove()
    ).rejects.toBeDefined();
  });
