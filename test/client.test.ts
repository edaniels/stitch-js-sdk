...
describe("RealmAdminClient", () => {
  ...
  it("should set the auth.requestOrigin field to what is specified in the options on create", async () => {
    ...
    expect(client.auth.requestOrigin).toEqual("mongodb-stitch-ui");
  });
});
...
describe("Auth login semantics", () => {
  ...
  beforeEach(async () => {
    ...
    apiKeySecret = apiKey;
    client = new RealmAdminClient({ baseUrl: serverUrl });
    await client.logout();
  });
...
describe("Auth", () => {
  ...
  it("should fail trying to do an authed action", async () => {
    ...
    const client = new RealmAdminClient();
    client.auth.clear();
    await expect(client.apps("foo").list()).rejects.toEqual(
      new RealmError("Must auth first", ErrUnauthorized)
    );
  });
});
...
const APIKEY_AUTH_URL =
  "https://realm.mongodb.com/api/admin/v3.0/auth/providers/api-key/login";
const LOCALAUTH_URL =
  "https://realm.mongodb.com/api/admin/v3.0/auth/providers/local-userpass/login";

const LIST_APPS_URL =
  "https://realm.mongodb.com/api/admin/v3.0/groups/foo/apps";
    ...
    describe(envConfig.name, () => {
      ...
      it("stores the refresh token after logging in", async () => {
        ...
        return testClient
          ...
          .then(async () => {
            const storedToken = await testClient.auth.storage.get(
              REFRESH_TOKEN_KEY
            );
            expect(storedToken).toEqual(testOriginalRefreshToken);
            expect(await testClient.auth.getAccessToken()).toEqual(
              testOriginalAccessToken
            );
      ...
      it("fetches a new access token if InvalidSession is received", async () => {
        ...
        return testClient
          ...
          .then(async () => {
            expect(testClient.auth.getAccessToken()).toEqual(
              validAccessTokens[0]
            );
          })
          .then(async () => {
