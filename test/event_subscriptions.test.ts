...
function comparePartialEventSubscription({
  id,
  name,
  type,
  disabled,
...
function compareFullEventSubscription({
  id,
  name,
  type,
  disabled,
...
describe("Event Subscriptions", () => {
  ...
  it("creating an event subscription should make it appear in list", async () => {
    ...
    const eventSubscription = await eventSubscriptions.create(
      buildEventSubscriptionData(fn.id)
    );
  ...
  it("fetching an event subscription returns the full model", async () => {
    ...
    const subscription = await eventSubscriptions
  ...
  it("can update an event subscription", async () => {
    ...
    await eventSubscriptions
      .eventSubscription(eventSubscription.id)
      .update(updatedEventSubscriptionData);

    const eventSubscriptionUpdated = await eventSubscriptions
      .eventSubscription(eventSubscription.id)
      .get();
  ...
  it("can resume an event subscription", async () => {
    ...
    const eventSubscriptionData = buildDBEventSubscriptionData(
      fn.id,
      mongodbService.id
    );
    const eventSubscription = await eventSubscriptions.create(
