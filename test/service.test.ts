...
const TEST_DB = Math.random()
  .toString(16)
  .substring(2);
const TESTNS1 = Math.random()
  .toString(16)
  .substring(2);
const TESTNS2 = Math.random()
  .toString(16)
  .substring(2);
...
describe("Services", () => {
  ...
  it("creating service should work", async () => {
    const newSvc = await services.create(
      new ServiceDesc({ name: "testsvc", type: "http" })
    );
  ...
  it("invalid create requests should fail", async () => {
    await expect(
      services.create(new ServiceDesc({ name: "testsvc", type: "invalid-svc" }))
    ).rejects.toBeDefined();
    await expect(
      services.create(
        new ServiceDesc({ name: "bad#name", type: "invalid-svc" })
      )
    ).rejects.toBeDefined();
  });
  ...
  it("fetching service config should work", async () => {
    ...
    const svcConfig = await services
      .service(newSvc.id)
      .config()
      .get();
  ...
  it("running service commands should work", async () => {
    ...
    const appObj = test.admin
      .apps(test.userData.group.groupId)
      .app(th.testApp.id);
    const providers = await appObj.authProviders().list();
    await appObj
      .authProviders()
      .authProvider(providers[0].id)
      .enable();
  ...
  it("creating rule should work", async () => {
    ...
    let rules = await services
      .service(newSvc.id)
      .rules()
      .list();
    expect(rules).toEqual([]);
    const newRule = await services
      .service(newSvc.id)
      .rules()
      .create(testRule);
    rules = await services
      .service(newSvc.id)
      .rules()
      .list();
  ...
  it("fetching rule should work", async () => {
    ...
    const newRule = await services
      ...
      .rules()
      .create(testRule);
    const fetchedRule = await services
      .service(newSvc.id)
      .rules()
      .rule(newRule.id)
      .get();
  ...
  it("creating an incoming webhook should work", async () => {
    ...
    let webhooks = await services
      .service(newSvc.id)
      .incomingWebhooks()
      .list();
    expect(webhooks).toEqual([]);

    const newWebhook = await services
      .service(newSvc.id)
      .incomingWebhooks()
      .create(testWebhook);
    webhooks = await services
      .service(newSvc.id)
      .incomingWebhooks()
      .list();
  ...
  it("fetching an incoming webhook should work", async () => {
    ...
    const newWebhook = await services
      ...
      .incomingWebhooks()
      .create(testWebhook);
    const fetchedWebhook = await services
      .service(newSvc.id)
      .incomingWebhooks()
      .incomingWebhook(newWebhook.id)
      .get();
  ...
  it("updating an incoming webhook should work", async () => {
    ...
    expect(fetchedWebhook.functionSource).toEqual(
      expectedWebhookWithUpdates.functionSource
    );
    expect(fetchedWebhook.options).toEqual(expectedWebhookWithUpdates.options);
    expect(fetchedWebhook.respondResult).toEqual(
      expectedWebhookWithUpdates.respondResult
    );
    expect(fetchedWebhook.runAsUserId).toEqual(
      expectedWebhookWithUpdates.runAsUserId
    );
    expect(fetchedWebhook.runAsUserIdScriptSource).toEqual(
      expectedWebhookWithUpdates.runAsUserIdScriptSource
    );
  });
